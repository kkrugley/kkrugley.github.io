...
document.addEventListener("DOMContentLoaded", () => {
    // Генерация 6 блоков с уникальными случайными HEX-кодами
    for (let i = 0; i < 6; i++) {
...
function checkCompatibility() {
    ...
    switch (scheme) {
        ...
        case "monochromatic": {
            const offsets = [-0.2, -0.1, 0.1, 0.2, 0.3];
            for (let i = 0; i < colors.length; i++) {
                if (i === mainColorIndex) continue;
        ...
        case "split-complementary": {
            const offsets = [-30, 30, -45, 45, -60];
            for (let i = 0; i < colors.length; i++) {
                if (i === mainColorIndex) continue;
                let offset = offsets[j % 5];
        ...
        case "square": {
            const offsets = [90, 180, 270, 360, 450];
            for (let i = 0; i < colors.length; i++) {
                if (i === mainColorIndex) continue;
...
// Отображение уведомления справа от кнопки "Check"
function showNotification(message, type) {
    const notification = document.getElementById("checkNotification");
...
function copyToClipboard(index) {
    const button = document.querySelector(`#block${index} .copy-btn`);
    const hexValue = document.querySelector(`#block${index} .hex-input`).value;

    navigator.clipboard.writeText(hexValue).then(() => {
      // Меняем текст на иконку галочки
      button.textContent = "✓";
      button.disabled = true; // Делаем кнопку неактивной на время
  
      // Возвращаем текст через 1 секунду
      setTimeout(() => {
